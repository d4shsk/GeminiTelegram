...
Никогда не используй Markdown-таблицы (со столбцами через |). Мобильные клиенты Telegram их плохо отображают. 
Вместо таблиц выводи данные в виде структурированных списков-карточек или в виде текста, разделенного по абзацам.

Текущие дата и время (МСК): {current_datetime}"""

SERIOUS_SYSTEM_PROMPT = """Ты — экспертный и объективный ИИ-ассистент. 
Отвечай максимально точно, структурированно и по делу. Не используй эмодзи без необходимости.

СТРОГОЕ ПРАВИЛО ФОРМАТИРОВАНИЯ:
Никогда не используй Markdown-таблицы (со столбцами через |). Мобильные клиенты Telegram их плохо отображают. 
Вместо таблиц всегда выводи данные в виде структурированных списков-карточек. 
...
async def handle_message(message: types.Message):
    ...
    for model_info in current_priority:
        ...
        if mode == "worker":
            ...
            tz_moscow = pytz.timezone("Europe/Moscow")
            current_datetime = datetime.now(tz_moscow).strftime("%d %B %Y, %H:%M МСК")
            system_instruction = SERIOUS_SYSTEM_PROMPT.format(current_datetime=current_datetime)
        ...
        try:
            if provider == "google":
                ...
                tz_moscow = pytz.timezone("Europe/Moscow")
                current_datetime = datetime.now(tz_moscow).strftime("%d %B %Y, %H:%M МСК")
                cf_system_text = system_instruction if system_instruction else f"Текущие дата и время (МСК): {current_datetime}"
                cf_history = [{"role": "system", "content": cf_system_text}]
                for msg in sessions.get(chat_id, []):
                    role = "assistant" if msg["role"] == "model" else msg["role"]
                ...
                tz_moscow = pytz.timezone("Europe/Moscow")
                current_datetime = datetime.now(tz_moscow).strftime("%d %B %Y, %H:%M МСК")
                gh_system_text = system_instruction if system_instruction else f"Текущие дата и время (МСК): {current_datetime}"
                gh_history.append({
                    "role": "system",
                    "content": gh_system_text
                })
                for msg in sessions.get(chat_id, []):
                    role = "assistant" if msg["role"] == "model" else msg["role"]
